      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
        {numbers.map((i) => (
          <Link key={i} to={`img/${i}`} className="bg-black/10 h-50">
            <img src={`/imgs/img_${i}.jpg`} className="w-full h-full object-cover object-center" />
          </Link>
        ))}
      </div>
      <button className="w-35 h-10 self-center bg-black text-white text-[11px] uppercase tracking-[1.65px] hover:border hover:text-black hover:bg-white cursor-pointer hover:border-black transition-all">
        View More
      </button>
    </section>
  );
